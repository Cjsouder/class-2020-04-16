...
A 1 unit increase in income decreases the probability of voting for a democrat.
...
tibble(`Error rate` = forest_fit$fit$err.rate[, "OOB"],
       Trees = 1:500) %>%
...
What is OOB error?
Out of Bag error - the chance of error at the maximum number of trees used (in this case, 500 trees has an
error rate of 25.34%)
...
nes %>%
  mutate(modal = 1,
         logistic = predict(logistic_fit, new_data = nes) %>% 
           pull(.pred_class),
         tree = predict(cart_fit, new_data = nes) %>% 
           pull(.pred_class),
         forest = predict(forest_fit, new_data = nes) %>% 
           pull(.pred_class)) 
